...
exports.extractHeroData = extractHeroData;
function extractHeroData(siteData) {
    ...
    if (!subtitle && siteData.pages && siteData.pages[0]) {
        ...
        if (firstPage.description)
            subtitle = firstPage.description;
            ...
            if (page.sitesections) {
                for (const section of page.sitesections) {
                    if (section._block === "hero_section" || section._block === "hero") {
                        ...
                        else if (section.background_image?.url)
                            image = section.background_image.url;
                        else if (section.hero_image?.url)
                            image = section.hero_image.url;
                    }
                }
            }
        }
