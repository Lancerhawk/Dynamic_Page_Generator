...
exports.extractDataByPath = extractDataByPath;
function calculateIntents(siteData) {
    const intents = [];
    ...
    if (siteData.pages) {
        siteData.pages.forEach((page, index) => {
            ...
            if (page.sitesections && page.sitesections.length > 0) {
                page.sitesections.forEach((section, sectionIndex) => {
    ...
    const uniqueIntents = intents.filter((intent, index, self) => index === self.findIndex((t) => t.title === intent.title));
...
function extractSinglePath(data, path) {
